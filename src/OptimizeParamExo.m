% Creates a param struct with exogenous variables
% from a previous parameter w/o exogenous variables

function [param] = OptimizeParamExo(param)

    %%%%%
    % Optimization Parameters
    %%%%%
    
    param.optInit = repmat(0.5,param.tmax,1); % This must match the type of the optimized variable
    % if optimized variable is a scalar, optInit is scalar, same for
    % vectors
    ...
    for i = 1:param.tmax
        ...
        if i == 1
            ...
            param.al(i) = param.al(i-1) / (1 - param.ga(i-1));
