...
class DamagesModel(object):
    ...
    def get_model_values(self, i, df):
        if i == 0:
            df.participation = np.tile(self.participation, (self.params.tmax + 1, 1)).transpose()
...
class IncommensurableDamages(DamagesModel):
    ...
    def get_model_values(self, index, df):
        if index == 0:
            df.participation = np.tile(self.participation, (self.params.tmax + 1, 1))
