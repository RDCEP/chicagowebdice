  ...
  if (!mclmcrInitialize())
    return false;
    {
        mclCtfStream ctfStream = 
            mclGetEmbeddedCtfStream((void *)(libdiceInitializeWithHandlers), 
                                    3882355);
        if (ctfStream) {
...
{
  return libdiceInitializeWithHandlers(mclDefaultErrorHandler, mclDefaultPrintHandler);
}
...
LIB_libdice_CPP_API 
void MW_CALL_CONV diceDriver(int nargout, mwArray& v, mwArray& p, const mwArray& source, 
                             const mwArray& target)
{
  mclcppMlfFeval(_mcr_inst, "diceDriver", nargout, 2, 2, &v, &p, &source, &target);
}
